...
type rootT struct {
	...
	SupportFiles    []string `cli:"supp" usage:"Any supplemental files needed for parsing.  Use more than one flag for more files."`
...
func main() {
	cli.Run(new(rootT), func(ctx *cli.Context) error {
		...
		for _, fileName := range argv.SupportFiles {
			supportFileName, _ := filepath.Abs(fileName)
			g.AddSupportFiles(supportFileName)
		}
